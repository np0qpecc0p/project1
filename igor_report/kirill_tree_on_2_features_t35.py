# Kirill's Decision Tree Classifier - Replicated on 2 Raw Features (Threshold = 35.0)
...
plt.savefig('igor_report/histogram_labels_t35.png')
plt.close()
...
# i want to plot distribution of each feature in mX matrix for each class (nrows=1, ncols=2 to avoid out of bounds)
...
for feat_idx, axis in enumerate(ax.flat):
    ...
    axis.legend()
plt.tight_layout()
plt.savefig('igor_report/feature_distributions_t35.png')
plt.close()
...
plt.savefig('igor_report/f1_heatmap_t35.png')
plt.close()
...
plt.savefig('igor_report/precision_heatmap_t35.png')
plt.close()
...
hA.bar(x = lfeatures_names, height = vFeatureImportance, color='teal')
hA.set_title('Features Importance of the Model')
hA.set_xlabel('Feature Name')
hA.set_ylabel('Importance')
plt.tight_layout()
plt.savefig('igor_report/feature_importance_t35.png')
plt.close()
