...
def run_feature_selection_t35():
    print("="*60)
    print("HYBRID FEATURE SELECTION: BINARY CLASSIFICATION AT THRESHOLD 35")
    print("="*60)
    ...
    print(f"-> Class balance (Thresh 35): {np.sum(vY==0)} Weak / {np.sum(vY==1)} Strong")
    ...
    plt.title("Top 20 RF Feature Importances (Threshold = 35.0)")
    plt.xlabel("RF Importance Score")
    plt.ylabel("Descriptor Name")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "raw_feature_importances_t35.png"), dpi=300)
    plt.close()
    ...
    print("\n" + "="*50)
    print("WINNING FEATURES (Threshold 35.0):")
    print(selected_features)
    print("="*50)

if __name__ == "__main__":
    run_feature_selection_t35()
